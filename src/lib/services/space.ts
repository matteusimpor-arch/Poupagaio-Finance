...
export const spaceService = {
  async getUserSpaces(userId: string): Promise<Space[]> {
    try {
      ...
      return await mockSupabase.db.getSpacesForUser(userId);
    } catch (err: any) {
  ...
  async ensurePersonalSpace(userId: string, spaceName: string = 'Pessoal'): Promise<Space | null> {
    try {
      ...
      // Ambiente mock isolado (somente quando Supabase não configurado)
      const existing = await mockSupabase.db.getSpacesForUser(userId);
      const personal = existing.find((s) => s.owner_id === userId && s.type === 'personal');
      if (personal) return personal;

      const newSpace = await mockSupabase.db.createSpace({
        name: spaceName,
        type: 'personal',
        owner_id: userId,
      });

      await mockSupabase.db.addSpaceMember({
        space_id: newSpace.id,
        user_id: userId,
        role: 'owner',
      });
