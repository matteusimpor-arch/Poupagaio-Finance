...
export const InstallmentCard: React.FC<InstallmentCardProps> = ({
  ...
  return (
    <div className="rounded-3xl border border-[#E8E4D5] dark:border-[#24312B] bg-white dark:bg-[#18211D] shadow-xs overflow-hidden transition-all duration-200 hover:border-[#16A66A]/30">
      {/* Cabeçalho da Compra */}
      <div className="p-5 sm:p-6 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          {/* Informações Principais */}
          <div className="flex items-start gap-3.5 min-w-0">
            <div className="w-10 h-10 rounded-2xl bg-[#075C45]/10 text-[#075C45] dark:bg-[#16A66A]/20 dark:text-[#78D9A6] flex items-center justify-center shrink-0 mt-0.5">
              {getCategoryIcon(purchase.category)}
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="text-base sm:text-lg font-bold font-display text-[#202724] dark:text-[#F7F4EA] truncate">
                  {purchase.description}
                </h3>
                <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-[#F7F4EA] dark:bg-[#101614] border border-[#E8E4D5] dark:border-[#24312B] text-[#5E6963] dark:text-[#95A39B]">
                  {purchase.category}
                </span>
                {purchase.isFullyPaid && (
                  <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border border-emerald-500/30">
          ...
          <div className="flex items-center gap-1.5 self-end sm:self-center">
            <button
              ...
              className="p-2 rounded-xl border border-[#E8E4D5] dark:border-[#24312B] hover:bg-black/5 dark:hover:bg-white/5 text-[#5E6963] dark:text-[#95A39B] hover:text-[#075C45] dark:hover:text-[#78D9A6] transition-colors cursor-pointer"
            >
            ...
            <button
              ...
              className="p-2 rounded-xl border border-[#E8E4D5] dark:border-[#24312B] hover:bg-rose-500/10 text-[#5E6963] dark:text-[#95A39B] hover:text-rose-600 transition-colors cursor-pointer"
            >
        ...
        <div className="space-y-1.5 pt-1">
          ...
          <div className="w-full h-2 rounded-full bg-[#F7F4EA] dark:bg-[#101614] overflow-hidden border border-[#E8E4D5] dark:border-[#24312B]">
            <div
        ...
        {purchase.notes && (
          <p className="text-xs italic text-[#5E6963] dark:text-[#95A39B] bg-[#F7F4EA]/50 dark:bg-[#121915] p-2 rounded-xl border border-[#E8E4D5] dark:border-[#24312B]">
        ...
        <div className="pt-2 border-t border-[#E8E4D5] dark:border-[#24312B] flex items-center justify-between">
          ...
          <Button
            ...
            className="text-xs font-semibold text-[#075C45] dark:text-[#78D9A6] hover:bg-[#075C45]/10 rounded-xl gap-1.5 cursor-pointer"
          >
            {isExpanded ? (
              <>
      ...
      {isExpanded && (
        <div className="bg-[#F7F4EA]/40 dark:bg-[#121915]/50 border-t border-[#E8E4D5] dark:border-[#24312B] p-4 sm:p-6 space-y-2.5">
          <h4 className="text-xs font-bold uppercase tracking-wider text-[#5E6963] dark:text-[#95A39B] pb-1">
            Parcelas Detalhadas
          </h4>

          <div className="divide-y divide-[#E8E4D5] dark:divide-[#24312B] rounded-2xl border border-[#E8E4D5] dark:border-[#24312B] bg-white dark:bg-[#18211D] overflow-hidden">
            {purchase.installments.map((inst) => {
              const isLoading = loadingInstallmentId === inst.id;
              return (
                <div
                  key={inst.id}
                  className="flex flex-col sm:flex-row sm:items-center justify-between p-3 sm:p-3.5 gap-2 hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors"
                >
                  {/* Identificação da Parcela e Vencimento */}
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-mono font-bold px-2 py-0.5 rounded-md bg-[#F7F4EA] dark:bg-[#101614] border border-[#E8E4D5] dark:border-[#24312B] text-[#202724] dark:text-[#F7F4EA]">
                      {inst.installment_number}/{purchase.installment_count}
                    </span>
                    <div>
                      <div className="text-xs font-semibold text-[#202724] dark:text-[#F7F4EA]">
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-3 self-stretch sm:self-auto">
                    ...
                    {inst.status === 'paid' ? (
                      <Button
                        ...
                        className="text-xs rounded-xl border-emerald-500/30 text-emerald-700 dark:text-emerald-400 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-300 dark:hover:bg-rose-950/20 dark:hover:text-rose-400 transition-colors cursor-pointer"
                      >
                      ...
                      <Button
                        ...
                        disabled={isLoading}
                        onClick={() => handleTogglePayment(inst)}
                        className="text-xs rounded-xl bg-[#075C45] hover:bg-[#075C45]/90 dark:bg-[#16A66A] dark:hover:bg-[#16A66A]/90 text-white font-semibold cursor-pointer"
                      >
