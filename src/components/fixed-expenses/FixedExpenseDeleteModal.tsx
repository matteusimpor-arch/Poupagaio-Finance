...
import { FixedExpenseWithStatus } from '../../types';
import { Button } from '../ui/button';
import { AlertTriangle, X } from 'lucide-react';
import { formatCurrency } from '../../lib/formatters';

interface FixedExpenseDeleteModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => Promise<void>;
  expense: FixedExpenseWithStatus | null;
  isDeleting: boolean;
}

export function FixedExpenseDeleteModal({
  isOpen,
  onClose,
  onConfirm,
  expense,
  isDeleting,
}: FixedExpenseDeleteModalProps) {
      ...
      <div className="w-full max-w-md bg-white dark:bg-[#18211D] border border-[#E8E4D5] dark:border-[#24312B] rounded-3xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-150">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E8E4D5] dark:border-[#24312B]">
          <div className="flex items-center gap-2 text-rose-600 dark:text-rose-400">
            ...
            <h2 id="delete-fixed-expense-title" className="text-base font-bold font-display">
              Excluir gasto fixo
            </h2>
          </div>
          <button
        ...
        <div className="p-6 space-y-4">
          <p className="text-xs sm:text-sm text-[#5E6963] dark:text-[#95A39B] leading-relaxed">
            Tem certeza que deseja excluir o gasto fixo{' '}
            <strong className="text-[#202724] dark:text-[#F7F4EA]">"{expense.description}"</strong> de{' '}
            <strong className="text-rose-600 dark:text-rose-400">{formatCurrency(expense.amount)}</strong>?
          </p>

          <p className="text-[11px] text-[#5E6963] dark:text-[#95A39B] bg-black/5 dark:bg-white/5 p-3 rounded-2xl">
            A exclusão removerá a despesa recorrente e seus históricos de pagamentos em todas as competências deste espaço.
          </p>
